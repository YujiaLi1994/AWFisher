...
N<-100
sigma_x<-rep(0.5,3)
sigma_y<-c(0.1,2,2,2,0.1,2,2,2,2,1)
sigma_c<-0.5
library(mvtnorm)
library(MASS)
library(parallel)
...
for(ind.sigma.mu in 1:length(sigma_mu_vector)){
  ...
  res<-mclapply(1:num.sim,function(ind.sim){
    set.seed(ind.sim)
    data<-Sim2D(N,sigma_x = sigma_x,sigma_mu = sigma_mu,sigma_y = sigma_y,sigma_c=sigma_c)
    x<-data$x
    y<-data$y
    x<-as.data.frame(x)
    z<-data$z
    ...
    res.origin<-Getpvalue(X = as.matrix(x),Y = y,Z = z)
    ...
    res.perm<-GetPermutationPvalue(as.matrix(r1),Y.residual,number.perm=500,ncores=1)
    Pvalue.permutation<-NULL
    for(i in 1:500){
      Pvalue.permutation<-rbind(Pvalue.permutation,res.perm$Pvalue[[i]])
    }
  ...
  file_name<-paste("Sim2E_hierarchical_sigma_mu=",sigma_mu,".Rdata",sep="")
